...
class PublicSiteTests(unittest.TestCase):
    ...
    def test_exports_json_csv_hashes_and_excludes_private_state(self) -> None:
        with TemporaryDirectory() as temp:
            ...
            self.assertEqual(manifest["supported_languages"], ["en", "zh-CN"])
            self.assertEqual(manifest["region_model_version"], "2026-08-14.v1")
            self.assertIn("eu-eea", manifest["region_presets"])
            self.assertEqual(manifest["counts"]["official_verified_resources"], 3)
            self.assertEqual(manifest["counts"]["community_candidates"], 0)
            ...
            english_home = (site / "index.html").read_text(encoding="utf-8")
            ...
            self.assertNotIn("按你的实际需求找资源", chinese_home)
            self.assertNotIn("Find resources for your use case", chinese_home)
            self.assertNotIn('./zh/scenarios/free-ai-api-no-card/', chinese_home)
            self.assertIn("<loc>https://ai-resource-radar.github.io/ai-resource-radar/zh/</loc>", (site / "sitemap.xml").read_text(encoding="utf-8"))
